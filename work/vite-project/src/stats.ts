    ...
    return {
        ...
        begin: function () {
            ...
            if (time >= prevTime + 1000) {
                _FPS = Math.round((frames * 1000) / (time - prevTime));
...
Stats.Panel = function (name: string, fg: string, bg: string) {
    ...
    canvas.style.cssText = "width:240px;height:144px";
    ...
    return {
        ...
        update: (value: number, maxValue: number) => {
            ...
            //const _text = `${Math.round(value)} ${name} (${Math.round(min)}-${Math.round(max)})`;
            const _text = `${Math.round(value)} ${name} (${_childLength}) `;
            context.fillText(_text, TEXT_X, TEXT_Y);
