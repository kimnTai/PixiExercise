...
export default class SpineDebug {
    spine;
    skeleton: any;
    list: any;
    ghs: any;
    ...
    init() {
        ...
        for (let i = 0, len = this.list.length; i < len; i++) {
            ...
            this.ghs[name] = name === "bones" ? new PIXI.Container() : new PIXI.Graphics();
            this.debugGroup.addChild(this.ghs[name]);
        }

        this.spine.addChild(this.debugGroup);
    }
    update() {
      

        this.scale = this.spine.scale.x || this.spine.scale.y || 1;
        this.lineWidth = 1 / Math.abs(this.scale);

        // 清除绘制
        for (let key in this.ghs) {
            if (this.ghs[key] instanceof PIXI.Graphics) {
                this.ghs[key].clear();
            } else if (this.ghs[key] instanceof PIXI.Container) {
                for (let len = this.ghs[key].children.length; len > 0; len--) {
                    this.ghs[key].children[len - 1].destroy({ children: true, texture: true, baseTexture: true });
                }
            }
        }
    ...
    drawClipping() {
        ...
        for (let i = 0, len = slots.length; i < len; i++) {
            ...
            let attachment = slot.getAttachment();
            if (!(attachment instanceof PIXI.spine.core.ClippingAttachment)) {
                continue;
            }
